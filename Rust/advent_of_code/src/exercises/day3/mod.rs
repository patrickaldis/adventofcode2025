...
// pub fn head_and_tails<T: Clone>(xs: &Vec<T>) -> Vec<(T, Vec<T>)> {
//     match xs {
...
// }
//
// pub fn find_max_jolt(xs: &Vec<u8>) -> Option<i32> {
...
//     use std::fs;
//
//     use crate::exercises::day3::parser::parse_full;
//
//     use super::*;
//
//     #[test]
...
//             fs::read_to_string("src/exercises/day3/input.txt").expect("expected to read file");
//
//         let (_rest, batteries) = parse_full(&test_input)
//             .unwrap_or_else(|err| panic!["error occured: {err}"]);
//
//         let final_res = batteries.iter().map(|x| find_max_jolt(*x));
//
